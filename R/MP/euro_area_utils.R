...
#
# Static regression:
#   ΔY_t = β MPS_t + γ(MPS_t × Frag_t) + δ Frag_t + X_t'θ + ε_t
#
# Local projections:
#   Y_{t+h} - Y_{t-1} = β_h MPS_t + γ_h(MPS_t × Frag_t) + X_t'θ_h + ε_{t+h}
#
...
extract_lp_irf_terms <- function(results, shock_var, state_var) {
  ...
  #   - gamma_h: coefficient on the shock × fragmentation interaction
  #   - variance of beta_h
  #   - variance of gamma_h
...
build_irf_state <- function(lp_terms, state_value, state_label, source_label) {
  ...
  #     IRF_h(state) = beta_h + gamma_h × state
  # -------------------------------------------------------------------------
  # INPUTS
...
run_ea_lps <- function(ea_reg_df, source_label, controls, max_h = 24, nw_lag = 6) {
  ...
  # extracts beta_h and gamma_h, builds conditional IRFs for 
  # low, average and high fragmentation,
  # constructs the differential IRF high minus low, and stores the interaction
  # coefficient path gamma_h.
  # -------------------------------------------------------------------------
  # INPUTS
  #     ea_reg_df: euro-area regression dataframe.
  #     source_label: label identifying the construction of euro-area variables.
...
build_static_marginal_effect <- function(reg_object, vcov_object, ea_reg_df, source_label) {
  ...
  #     dΔY_t / dMPS_t = beta_MPS + gamma × frag_ea_c
